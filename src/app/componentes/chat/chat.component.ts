...
export class ChatComponent implements OnInit{
  ...
  homeChat(){
    ...
    if(this.chatService.idChat != ""){
      ...
      if(idChat != undefined){
        var idUser = this.obtainUser(this.chatService.idChat);
        // this.swapPosition(this.users, 0, 1);
        this.updateData(this.users[idUser]['fotoPerfil'], this.users[idUser]['nombreCompleto'], this.users[idChat]['messages'], idChat);
        setTimeout( () => {
        ...
        this.informationChat = this.users[idUser];  
      }
      else{
        this.obtainUserInformation(this.chatService.idChat, () => {
          this.updateData(this.users[0]['fotoPerfil'], this.users[0]['nombreCompleto'], this.users[0]['messages'], 0);
  ...
  updateData(actualPhoto: string, actualName : string, messages: Array<any>, idChat: number) : void{
    this.actualMessages = this.messages[idChat]['messages'];
  ...
  obtainUser(idEmpresa: String): number | undefined{ 
    for(var i = 0; i < this.users.length; i++){
      if(idEmpresa == this.users[i]['_id']){
        return i;
      }
    }
    return undefined;
